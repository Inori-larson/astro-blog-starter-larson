...
export const DEFAULT_SITE_SETTINGS: SiteSettings = {
	...
	heroTaglineDark: '圣杯问答，今夜开始',
	heroBio: '一名爱写代码也爱写文字的开发者。在这里记录技术笔记、项目复盘与生活随想，把踩过的坑写成文字，分享给同样在路上的人。',
	aboutRole: '开发者 · 写字的人 · 终身学习者',
	...
	aboutContactTitle: '想和我聊聊？',
	aboutContactDesc: '无论是技术交流、合作意向，还是单纯打个招呼，都欢迎随时联系。',
...
const TEXT_KEYS = {
	...
	aboutContactEmail: 'site.about_contact_email',
} as const;
...
export function renderAboutParagraph(text: string, name: string): string {
	const escaped = text
		...
		.replace(/>/g, '&gt;')
		.replace(/"/g, '&quot;');
	return escaped.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>').replaceAll('{name}', name);
}

export type NormalizeResult = { ok: true; value: SiteSettings } | { ok: false; error: string };

/** 值为纯字符串的设置键（供校验循环收窄类型） */
...
export function normalizeSiteSettings(input: unknown): NormalizeResult {
	...
	for (const { field, label, max } of REQUIRED_TEXT) {
		const raw = obj[field];
...
/** 序列化为 settings 表行（批量 upsert 用） */
export function serializeSettings(s: SiteSettings): { key: string; value: string }[] {
	return [
		...Object.entries(TEXT_KEYS).map(([field, key]) => ({ key, value: String(s[field as keyof SiteSettings]) })),
		{ key: BADGE_KEY, value: JSON.stringify(s.aboutBadges) },
		{ key: TIMELINE_KEY, value: JSON.stringify(s.aboutTimeline) },
	];
}
